...
@click.option("-o", "--outfile", default="results.csv")
@click.option("-h", "--history_file", default="results.csv")
def main(
    ...
    classifier = ClassifierApp(imgdir, outfile, history_file)
    try:
        app.exec()
...
def get_image_paths(imgdir):
    cmd = f'find {imgdir} -name "*snr*.png"' % (imgdir)
